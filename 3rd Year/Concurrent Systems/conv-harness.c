            ...
            for (int xy = 0; xy < kernelSize; xy++)
            {
                ...
                for (m = 0; m < nkernels; m++)
                {
                    ...
                    if ((kend - index) == 4 && index % 4 == 0)
                    {
                        ...
                        float var1 = (cachedImage[kernel->channel_numbers[index]] * kernel->values[index]);
                        float var2 = (cachedImage[kernel->channel_numbers[ind1]] * kernel->values[ind1]);
                        float var3 = (cachedImage[kernel->channel_numbers[ind2]] * kernel->values[ind2]);
                        float var4 = (cachedImage[kernel->channel_numbers[ind3]] * kernel->values[ind3]);
                        ...
                        float outputTest = output[m][h][w];
                        //output[m][h][w] += test;
                        //printf("CURRENT INDEX %d\n", index);
                        output[m][h][w] += (cachedImage[kernel->channel_numbers[index]] * kernel->values[index]);
                        //printf("CURRENT INDEX %d\n", index + 1);
                        output[m][h][w] += (cachedImage[kernel->channel_numbers[ind1]] * kernel->values[ind1]);
                        //printf("CURRENT INDEX %d\n", index + 2);
                        output[m][h][w] += (cachedImage[kernel->channel_numbers[ind2]] * kernel->values[ind2]);
                        //printf("CURRENT INDEX %d\n", index + 3);
                        output[m][h][w] += (cachedImage[kernel->channel_numbers[ind3]] * kernel->values[ind3]);

                        if(output[m][h][w] - outputTest != test){
                            printf("here");
                        }
                    }
                    else
